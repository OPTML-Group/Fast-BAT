...
class APGD(Attack):
    ...
    Arguments:
        ...
        loss (str): loss_pgd function optimized ('ce', 'dlr' supported, DEFAULT: 'ce')
    ...
    Examples::
        >>> attack = torchattacks.APGD(model, norm='Linf', eps=8/255, steps=100, n_restarts=1, seed=0, loss_pgd='ce', eot_iter=1, rho=.75, verbose=False)
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.loss == 'ce':
            ...
            raise ValueError('unknowkn loss_pgd')
        ...
        for i in range(self.steps):
            ...
            if self.verbose:
                print('iteration: {} - Best loss_pgd: {:.6f}'.format(i, loss_best.sum()))
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        if not best_loss:
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        if self.verbose:
                            print('restart {} - robust accuracy: {:.2%} - cum. time: {:.1f} s'.format(
                                counter, acc.float().mean(), time() - startt))
            ...
            for counter in range(self.n_restarts):
                ...
                if self.verbose:
                    print('restart {} - loss_pgd: {:.5f}'.format(counter, loss_best.sum()))
